...
from OCC.Core.Geom2dGcc import Geom2dGcc_QualifiedCurve, Geom2dGcc_Circ2d2TanRad
...
def writeOCCShape(shape, outfile, outformat='step'):
    ...
    if outformat=='step':
        ...
        raise ValueError(
            "output format not known (allowed values: 'iges', 'step', 'stl' or 'vrml')"
        )
...
class GearWheel:
    ...
    def _make3dGeomFrom2dGeom(self, geom2d_object, z_coord_of_plane=0.0):
        ...
        # convert offset curve          
            ...
            # In some pythonOCC builds BasisCurve() returns a handle with
            # .GetObject(), in others it returns the curve object directly.
            if hasattr(basis_curve2d, "GetObject"):
                basis_bspline2d = basis_curve2d.GetObject()
            else:
                basis_bspline2d = basis_curve2d

            # Convert 2D bspline to 3D bspline; in pythonocc-core we can pass
            # the Geom_BSplineCurve directly to Geom_OffsetCurve.
    ...
    def _makeFormWire(self):
        ...
        for condition in [root_circle_condition, root_rounding_condition, involute_condition,  \
            ...
            if count_nonzero(condition)>1:
                ...
                # make edge from segment curve. Depending on pythonOCC version,
                # Curve() may return a handle or a bare Geom2d_Curve. Use
                # GetObject() only if available.
                curve2d = curve.Curve()
                if hasattr(curve2d, "GetObject"):
                    curve2d = curve2d.GetObject()

                # Build a 3D curve and pass it directly to BRepBuilderAPI_MakeEdge
                curve3d = self._make3dGeomFrom2dGeom(curve2d)
                edge = BRepBuilderAPI_MakeEdge(curve3d, start_vertex, end_vertex).Edge()
